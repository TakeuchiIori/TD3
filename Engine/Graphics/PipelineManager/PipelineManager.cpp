...
void PipelineManager::CreatePSO_Animation()
{
	...
	// アニメーション
	rootParameters[8].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE; // ストラクチャーバッファー
	rootParameters[8].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;
	rootParameters[8].DescriptorTable.pDescriptorRanges = descriptorRangeBone;
	rootParameters[8].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeBone);
